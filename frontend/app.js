// Hard-coded API URL to avoid environment variables
...
class ChatInterface {
    constructor() {
        ...
        if (typeof marked !== 'undefined') {
            // Initialize marked options if the library is available
            marked.setOptions({
                breaks: true,  // Convert \n to <br>
...
// Create generateMealPlan as a regular function
function generateMealPlan() {
    try {
        ...
        makeApiRequest('/generate-meal-plan', mealPlanRequest)
            .then(response => {
                if (response.mealPlan) {
                    localStorage.setItem('mealPlan', response.mealPlan);
        ...
        if (response.ok) {
            ...
            if (mealPlanElement && data.meal_plan) {
                if (typeof marked !== 'undefined') {
                    ...
                    mealPlanElement.textContent = data.meal_plan;
                }
            } else if (mealPlanElement) {
        ...
        if (response.mealPlan) {
            ...
            if (mealPlanElement) {
                if (typeof marked !== 'undefined') {
                    ...
                    mealPlanElement.textContent = response.mealPlan;
                }
            }
            // Hide the tweak form
            const tweakForm = document.getElementById('tweak-form');
            if (tweakForm) {
                tweakForm.style.display = 'none';
            }
        }
    } catch (error) {
...
window.showTweakForm = showTweakForm;
window.submitTweakRequest = submitTweakRequest;